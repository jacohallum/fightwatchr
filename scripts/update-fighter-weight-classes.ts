...
async function updateFighterWeightClasses() {
  console.log('🔄 Starting weight class update for last 25 years...\n')
  ...
  console.log(`📊 Found ${fightersToUpdate.length} fighters needing updates\n`)
  ...
  const athleteCache = new Map<string, any>() // Cache ESPN athlete data by ref URL
      ...
      try {
        ...
        for (const comp of eventData.competitions || []) {
          for (const competitor of comp.competitors || []) {
            ...
            // Skip if already fetched this athlete
            ...
            try {
              ...
              if (dbFighter) {
                ...
                if (weightClass) {
                  ...
                  console.log(`  ✅ ${dbFighter.firstName} ${dbFighter.lastName} → ${weightClass}`)
